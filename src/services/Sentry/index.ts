...
const isWhiteListError = (errorCode: string) => Boolean(errorCode && API_ERROR_WHITE_LIST_HASH[errorCode]);
const isWhiteListStatus = (errorStatus: number) => API_STATUS_WHITE_LIST.indexOf(errorStatus) !== 0;
...
export const captureApiError = (error: any) => {
  ...
  if (method || url || error_code || status) {
    messageForSentry = `${method || ""} ${status || ""} ${error_code || ""} ${url || ""}`;
    errorForSentry.message = messageForSentry;
    fingerprint = [`${method || ""}`, `${status || ""}`, `${error_code || ""}`, `${url || ""}`];
  }
